...
const AUDIO_PREFS_KEY = 'archistudio_ambient_prefs';

export function AmbientAudio() {
  // Load saved prefs — default volume 30, default playing OFF
  const savedPrefs = (() => {
    try { return JSON.parse(localStorage.getItem(AUDIO_PREFS_KEY) || '{}'); } catch { return {}; }
  })();
  const [isPlaying, setIsPlaying] = useState(false);
  const [volume, setVolume] = useState<number>(savedPrefs.volume ?? 30);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(AUDIO_PREFS_KEY, JSON.stringify({ volume }));
  }, [volume]);
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          onClick={togglePlay}
          disabled={!isLoaded}
          className={`h-12 w-12 rounded-full shadow-2xl transition-all duration-300 border-2 ${isPlaying
