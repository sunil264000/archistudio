...
serve(async (req) => {
  ...
  try {
    ...
    let requestHeaders: HeadersInit = { Accept: "application/pdf" };
    if (rangeHeader) requestHeaders = { ...requestHeaders, Range: rangeHeader };
    ...
    if (!sourceResponse.ok && sourceResponse.status !== 206) {
      ...
      console.error("Preview stream upstream failure:", reason);
