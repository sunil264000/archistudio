...
// Strip exports and interfaces
content = content.replace(/export\s+interface\s+Course\s*{[^}]*}/g, '');
content = content.replace(/export\s+/g, '');
content = content.replace(/:\s*Course\[\]/g, '');
content = content.replace(/:\s*Record<string,\s*string>/g, '');

// Evaluate the string to get objects
const sandbox = {};
try {
    eval(content + '\nObject.assign(sandbox, { courseCategories, courses });');
} catch (e) {
    console.error("Eval error:", e);
    process.exit(1);
}

const { courseCategories, courses } = sandbox;
...
fs.writeFileSync('C:/Users/kssun/.gemini/antigravity/brain/a99b35b2-707d-44bc-b16d-ce8b23758258/migrate_courses.sql', sql);
